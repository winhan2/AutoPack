...
"""
    ...
    # 定义'pack'函数
    def pack(self, route):
        with open('..\AutoPack.bat', mode='wt', encoding='utf-8') as apfw:
            self.logger_obj.info('AutoPack.bat文件创建（打开并全部清除）成功')
            apfw.write(f'{route[:2]}\n')
            apfw.write(f'cd {os.path.split(route)[0]}\n')
            apfw.write(f'pyinstaller -F {route}\n')
            self.logger_obj.info('AutoPack.bat文件写入完成')
        self.logger_obj.info('AutoPack.bat文件关闭成功')
        os.system('..\AutoPack')
        self.logger_obj.info('AutoPack.bat文件执行成功')
        print(f'AutoPack.bat({os.path.split(route)[1]}打包用文件)在{os.path.split(route)[0]}路径下')
